        ...
        for epoch in range((self.nb_epochs-epoch_start)):
            ...
            for _, sequence_data in enumerate(train_loader):
                ...
                for _, frame in enumerate(frame_dataloader):
                    ...
                    elif self.model_name == 'RC-RODNet':
                        ra_outputs = self.net(img_data, ra_data)
                        rd_outputs = 0
                        ra_outputs = ra_outputs.to(self.device)
                    else:
            ...
            test_metrics = self.tester.predict(self.net, test_loader, add_temp=add_temp)
            ...
            if self.model_name == 'RC-RODNet':
                print('[Epoch {}/{}] Test losses: '
                      'RA={}'.format(epoch + epoch_start + 1,
                                     self.nb_epochs,
                                     test_metrics['range_angle']['loss']))
                print('[Epoch {}/{}] Test Prec: '
                      'RA={}, RA_miou={}'.format(epoch + epoch_start + 1,
                                                 self.nb_epochs,
...
#
#                 if self.model_name == 'RC-RODNet':
#                     self.visualizer.update_multi_val_metrics2(val_metrics, iteration)
...
#                 if val_metrics['range_angle']['miou'] > (best_val_miou):
#                     best_val_miou = val_metrics['range_angle']['miou']
#
#                     start_time = time.time()
