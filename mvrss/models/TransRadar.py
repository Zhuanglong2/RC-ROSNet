...
if __name__ == '__main__':
    ...
    rd    = torch.rand((1, 1, 5, 256, 64), device='cpu')
    ra    = torch.rand((1, 1, 5, 256, 256), device='cpu')
    ad    = torch.rand((1, 1, 5, 256, 64), device='cpu')

    net = TransRad(n_classes=4,
                      n_frames=5,
                      depth = 8,
                      channels = 64,
                      deform_k = [3, 3, 3, 3, 3, 3, 3, 3])
    ...
    # FLOPs = 27.6G
    # params = 148.3M
    flops, params = profile(net, inputs=(rd, ra,ad,))
