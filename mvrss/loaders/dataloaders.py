...
class CarradaDataset(Dataset):
    ...
    def __init__(self, dataset, annotation_type, path_to_frames, process_signal,
                 n_frames, img_w_size, img_h_size,transformations=None, add_temp=False):
    ...
    def __getitem__(self, idx):
        ...
        for frame_name in frame_names:
            if self.process_signal:
                try:        #Carada数据集的RGB图像不全
                    path = os.path.join(self.path_to_frames,'camera_images', frame_name + '.jpg')
                    img = cv2.imread(path)
                    img = cv2.resize(img, (self.img_w_size, self.img_h_size))
                except:
                    path = self.last_path
                    img = cv2.imread(path)
                    img = cv2.resize(img, (self.img_w_size, self.img_h_size))
                ...
                self.last_path = path
                img_matrix = np.array(img).transpose(2, 0, 1)
                img_matrix = np.expand_dims(img_matrix, 1)
        ...
        if self.add_temp:
            if isinstance(self.add_temp, bool):
                ...
                ad_frame['matrix'] = np.expand_dims(ad_frame['matrix'],
                                                    axis=self.add_temp)
        img_matrix = np.concatenate(img_matrices, axis=1)
        img_frame = {'img': img_matrix}

        frame = {'rd_matrix': rd_frame['matrix'], 'rd_mask': rd_frame['mask'],
                 'ra_matrix': ra_frame['matrix'], 'ra_mask': ra_frame['mask'],
                 'ad_matrix': ad_frame['matrix'], 'img'    : img_frame['img']}
